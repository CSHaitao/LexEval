...
class Baichuan_chat_generator(model_generator):
    def __init__(self, f_path, is_few_shot, device, is_vllm, model_path, model_name, few_shot_path, tensor_parallel_size, gpu_memory_utilization):
        ...
        '''
        super(Baichuan_chat_generator, self).__init__(f_path, is_few_shot, device, is_vllm, model_path, few_shot_path, tensor_parallel_size, gpu_memory_utilization)
    ...
    def generate_output(self, q_type, tokenizer, model, batch_size=None):
        ...
        for instruction, input_text, answer in tqdm(zip(instruction_ls, input_text_ls, answer_ls), total=len(input_text_ls)):
            ...
            try:
                config = GenerationConfig(max_new_tokens=20, do_sample=False) if q_type == "multiple_choice" else GenerationConfig(max_new_tokens=512, do_sample=False)
