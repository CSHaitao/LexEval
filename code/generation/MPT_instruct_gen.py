...
class MPT_instruct_generator(model_generator):
    def __init__(self, f_path, is_few_shot, device, is_vllm, model_path, model_name, few_shot_path, tensor_parallel_size, gpu_memory_utilization):
        ...
        '''
        super(MPT_instruct_generator, self).__init__(f_path, is_few_shot, device, is_vllm, model_path, few_shot_path, tensor_parallel_size, gpu_memory_utilization)
    ...
    def generate_output(self, q_type, tokenizer, model, batch_size=None):
        '''
        Generate the output for MPT_7B_instruct, using the suitable prompt template
        '''
        ...
        if self.is_vllm:
            ...
            for idx in range(0, len(instruction_ls), batch_size):
                ...
                for i in range(len(instruction)):
                    ...
                    current_prompt = PROMPT_FOR_GENERATION_FORMAT.format(instruction=current_prompt)
            ...
            for instruction, input_text, answer in tqdm(zip(instruction_ls, input_text_ls, answer_ls), total=len(input_text_ls)):
                ...
                try:
                    with torch.autocast(model.device.type, dtype=torch.bfloat16):
                        response = pipe(prompt, max_new_tokens=20, do_sample=False, use_cache=True)[0]["generated_text"][len(prompt):] if q_type == 'multiple_choice' else pipe(prompt, max_new_tokens=512, do_sample=False, use_cache=True)[0]["generated_text"][len(prompt):]
                except:
                    num_unsuccess += 1
                    print("Fail to answer")
                    response = "未成功回答"
